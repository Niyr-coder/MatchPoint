-- Migration 033: Fix score_bracket_match return type (B5/B6)
--
-- The function was declared with RETURNS TABLE(winner_new_rating INT, loser_new_rating INT).
-- Ratings are stored as NUMERIC(4,2) (e.g. 3.75), so ::INT casts in the RETURN QUERY
-- silently truncated decimals. This matters especially for the Pickleball DUPR-style
-- rating system where fractional deltas (0.25 / 0.15) are meaningful.
--
-- CREATE OR REPLACE cannot change a function's return type in PostgreSQL.
-- We must DROP and recreate. Permissions from migration 020 are re-applied below.

DROP FUNCTION IF EXISTS public.score_bracket_match(UUID, UUID, UUID, UUID, INT, INT, TEXT, BOOLEAN);

CREATE FUNCTION public.score_bracket_match(
  p_match_id       UUID,
...
RETURNS TABLE(
  ...
  next_match_id     UUID,
  winner_new_rating NUMERIC,   -- was INT; ratings are NUMERIC(4,2) (e.g. 3.75)
  loser_new_rating  NUMERIC    -- was INT
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Return NUMERIC ratings (was ::INT — truncated 3.75 to 3, etc.)
  RETURN QUERY SELECT true, v_next_match_id, v_winner_new_rat, v_loser_new_rat;
...
-- Re-apply permissions (originally set in migration 020; function was dropped above)
DO $$
BEGIN
